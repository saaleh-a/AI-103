...
- SDK implementation uses `SequentialBuilder`, `participants()`, `build()`, `run_stream`, async event iteration, and `WorkflowOutputEvent`. (SRC-245 L240–242)

## How it works

The pattern enforces an ordered dependency chain. (SRC-245 L220–221) Each stage adds or refines something that the next stage needs. (SRC-245 L223–227) Predictability is useful when agent performance, delays, or failures can be handled without stopping the whole process. (SRC-245 L228)
...
- **Sequential orchestration** — a fixed-order agent pipeline where each output becomes the next input. (SRC-245 L220–221)
...
- **Inference:** Choose sequential orchestration when wording emphasizes fixed order, dependencies, staged refinement, or pipelines. (SRC-245 L220–228)
- **Inference:** Reject sequential orchestration when the task requires parallelism, dynamic routing, backtracking, or free collaboration. (SRC-245 L230–235)
...
- **Stale-risk:** Builder and event names reflect the SDK at capture time. (SRC-245 L240–242)
- Uncorrected early-stage failure can contaminate downstream processing. (SRC-245 L230)

## Relation to other sources

- [[src-231-understand-agent-orchestration]] defines sequential orchestration as passing output from one agent to the next in fixed order. (SRC-231 L251; SRC-245 L220–244)
- [[src-241-concurrent-orchestration]] is the opposite pattern for independent parallel stages. (SRC-241 L220–237; SRC-245 L230–235)
...
- [[microsoft-agent-framework]] — the SDK supplies the builder and workflow execution. (SRC-245 L240–242)
